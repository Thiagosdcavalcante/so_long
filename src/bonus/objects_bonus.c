...
/*   Updated: 2024/03/25 20:57:25 by tsantana         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	objects_start(mlx_t *window_open, t_items *map_obj, t_sizes szs)
{
	...
	mlx_resize_image(map_obj->baseboard.img, 1920, 60);
	...
	mlx_image_to_window(window_open, map_obj->baseboard.img, 0, 930);
}
...
int	start_items(t_game *gm)
{
	...
	init_villain(gm->connect_mlx, gm->sizes, &gm->items);
	print_wall_exit(gm->connect_mlx, gm->sizes.tile, &gm->items, gm->map);
	make_player(gm->map, &gm->plr);
	gm->items.player.texture = mlx_load_png("assets/pigfront00.png");
